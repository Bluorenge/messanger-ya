...
export default function DataPicker({ onDateChange }: { onDateChange: any }) {
    function onChange(date: DatePickerValueChangeDetails) {
        if (date.value.length === 0) {
            onDateChange(null);
            return;
        }

        const { day, month, year } = date.value[0];
        const datesString = `${day}.${month}.${year}`;
        onDateChange(datesString);
    }
